...
Produces a time-stepped map, in sky coordinates, of stellar targets, planets, keepout
regions, and, if a DRM is provided (see below), detections, characterizations, and slews.
Output is to a movie (`.mp4`), or individual frames (`.png`).  Also, some cumulative
keepout summary statistics, and plots, can optionally be generated.

DRM or observing-tour display:  If pickles summarizing a DRM are supplied (`--drm`), then
the observing tour is loaded and the observations are shown. It is assumed that `--drm`
and `--spc` are used together.
