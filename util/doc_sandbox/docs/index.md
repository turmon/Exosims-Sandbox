...
We show HWO here for example, other simulations have
the same layout.
...
Graphical outputs are generated by Python/matplotlib scripts
in `util/plot_drm_gallery`.
...
Currently, about 50 files are generated, almost all PNG image files.
You can use `make S=(SCRIPT) graphics-extra` to make a deeper set of
plots that are not always useful.
These plots will be linked into the summary web page automatically 
the next time it is regenerated, e.g. with `make S=(SCRIPT) html-only`. 

See the [binned ensemble plot gallery](binned-ensemble-gallery.md).

### Graphical Output Files: Tours
  
Other plots show the tour of characterizations
made by a starshade mission.
