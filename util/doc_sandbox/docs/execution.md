...
Python scripts that generally only
...
*  `html-ensure`: ensure httpd server is running (starts new one if needed)

*  `html-stop`: stop httpd server

*  `html-start`: start a new httpd server
...
The server typically can stay up until its own job is killed.
(The servers on local HPC systems are auto-restarted by `cron` so the
servers there will restart automatically.)
