...
progname=`basename $0`
USAGE="Usage: $progname [-d] [-o OP] in_tmpl dest_tmpl"

# get options
developer_path=0
mode_arg=
while getopts "do:" o; do
    case "$o" in
	d)    developer_path=1;;
	o)    mode_arg="$mode_arg;op=$OPTARG";;
	[?])  echo "$USAGE" 1>&2
	      exit 2;;
    esac
done
...
# if this file is present, skip the graphics
skip_fn=$(printf $in_tmpl skip txt)
if [ -r $skip_fn ]; then
    echo "${progname}: Skipping graphics in $(dirname $in_tmpl)."
    exit 0
fi
