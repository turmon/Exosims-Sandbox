...
Also for `-i`, any intermediate `index.html` files between `sims/index.html` and
`sims/SIM/index.html` will also be made; this is needed when SIM is nested ("families").

## Options

+  `-i` => generate intermediate `index.html` files after generating any named SIM indexes.
...
* The output is written to the file `sims/SIM/index.html`.
...
* The generated HTML file links directly to whatever graphical outputs are present
  below the `sims/SIM/...` directory.  Use `make S=SIM html` to make the graphics
  files and then call this script to make the graphics viewable through the webpage.

## HTTP server

To see the HTML files and the images they point to, an HTTP server
must be started to send them to a browser.  We start a server listening on a
non-standard port, and the browser is directed to that port with a special URL
qualifier.  The server invocation wrapper is `html-serve.sh`, and it can be invoked by:
